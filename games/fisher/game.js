var update = function() {
	...
	for (var i = 0; i < fishes.length; i++) { // Waypoint mechanic
		if (hook.reelIn && fishes[i].x == hook.x && (fishes[i].y == hook.y || fishes[i].y == hook.y - 1 || fishes[i].y == hook.y + 1)) fishes[i].caught = true; 
		if (fishes[i].x == 12 && fishes[i].y == 0) fishes[i].onBoat = true;
		if (fishes[i].caught) {
			fishes[i].x = hook.x;
			fishes[i].y = hook.y;
		} else {
			if (fishes[i].x < fishes[i].waypoints[fishes[i].point+2])
				fishes[i].x++; if (fishes[i].fishImage.src != "img/fish_right.png") { fishes[i].fishImage.src = "img/fish_right.png";
		} 
			if (fishes[i].x > fishes[i].waypoints[fishes[i].point+2]) {
				fishes[i].x--;
				if (fishes[i].fishImage.src != "img/fish_left.png") { fishes[i].fishImage.src = "img/fish_left.png";
			}
		}
...
function reelIn() { sleepTime /= 10; hook.reelIn = true; music.pause(); reel.play(); }
