...
function pFrame(x, y, z)	{ document.getElementsByName(document.querySelectorAll('[id=nav]')[x].name)[0].src=`photos/framed_${document.querySelectorAll('[id=nav]')[x].name}${z==1?'_forward.gif':z==2?'_reverse.gif':'.gif'}`;pSound(y); } // ${i!=0&&i%2==0?'<br><br>':''}
function sMenu(o, m)		{ document.getElementById('menu').innerHTML='';menu=!menu;for(var i=0;i<items[o].length;i++){document.getElementById('menu').innerHTML+=menu?(m==0?`<a style='position:relative;padding:13em;' href='${items[o][i]}'><img id='menu' src='${items[o][i]}/logo.png'/></a>`:m==1?`<video onclick='sMenu(${o},1)' style='z-index:13;' id='video_background' autoplay><source src='movies/${items[o][i]}.mp4'></video>`:m==2?`<img src='load.jpg' onload='window.location.href=items[${o}][${i}];'/>`:''):'';}if(!track.paused)pTape();}
