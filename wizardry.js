const sounds = [ [ new Audio('vibrations/horn_forward.ogg'   ), new Audio('vibrations/horn_reverse.ogg')    ],
      ...
      items  = [ ['+'                                         ], ['//13bob.net/words'  ], ['fisher', 'slingshoot', 'ptolemy'],
                 ['//rym.13bob.net', '//letterboxd.13bob.net' ], ['//twitter.13bob.net'], ['-'                              ] ],
...
sMenu = (o, m) => { menu = !menu,
    ...
    for (let i = [] - []; i < items[o].length; i++) {
        ...
        document.getElementById('menu').innerHTML += menu ? (
            m == [] - [] ? `<a style='padding: 13em;' href='${items[o][i]}'><img style='${i > 2 ? `margin-top: ${(400 * fTop) - 1000}px;` : '' }' id='menu' src='${items[o][i].trim('//')}/logo.png'/></a>${i != [] - [] && i % 2 == [] - [] ? '<br>' : '' }` :
