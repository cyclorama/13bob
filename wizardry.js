...
function sMenu(o, m)		{ document.getElementById('menu').innerHTML='';menu=!menu;var fTop=0;for(var i=0;i<items[o].length;i++){fTop+=(i!=0&&i%3==0);document.getElementById('menu').innerHTML+=menu?(m==0?`<center><img onclick='window.location.href=${items[o][i]};' style='${i>2?'padding-top:'+400*fTop+'px;':''}' id='menu' src='${items[o][i]}/logo.png'/>${i!=0&&i%2==0?'<br>':''}</center>`:m==1?`<video onclick='sMenu(${o},1)' style='z-index:13;' id='video_background' autoplay><source src='movies/${items[o][i]}.mp4'></video>`:m==2?`<img src='load.jpg' onload='window.location.href=items[${o}][${i}];'/>`:''):'';}if(!track.paused)pTape();}
