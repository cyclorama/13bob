!async function (title, html, colours, sleep) {
    ...
    for (let pos = 0; pos < title.length + colours.length;) { html = '';
        ...
        await sleep(35).then(() => h1.innerHTML = `<a href="//13bob.net/">${title.substring(0, pos - title.length - 2)}${html}${title.substring(pos)}</a>`, pos++);
    }
