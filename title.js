!async function (title, position, html, colours, sleep) {
    ...
    await sleep(([+!+[]]+[+[]])*([+!+[]]+[+[]])*([+!+[]]+[+[]])).then(() => h1.style.animation = 'none');
    ...
    while (position < ([+!+[]]+[+[]])+(!+[]+!+[])) { html = '';
        ...
        await sleep(([+!+[]]+[+[]])*3+(([+!+[]]+[+[]])/2)).then(() => h1.innerHTML = `<a href="//13bob.net/">${title.substring(+[], position - title.length - (!+[]+!+[]))}${html}${title.substring(position)}</a>`), position++;
    }
