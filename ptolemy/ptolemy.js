window.onload = () => {
    ...
    function render() {
        ...
        [[pDist, qDist, rDist], [qDist, pDist, rDist], [rDist, qDist, pDist]].forEach((cond, i) => {
            if (cond[0] > cond[1] && cond[0] > cond[2]) { const base = { y : n => { return i != 0 ? (n ? 1 : 2) : (n ? 2 : 1) }, col : n => (i + n) % 3 };
                ...
                drawRectangle(-80      + centerX +   50, centerY - (cond[base[1]]) / 8 , 50, cond[base.y(1)] / 4, colours[base.col(2)]);
            }
        });
