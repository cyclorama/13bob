window.onload = () => {
    ...
    function render() {
        ...
        [[pDist, qDist, rDist], [qDist, pDist, rDist], [rDist, qDist, pDist]].forEach((cond, i) => {
            if (cond[0] > cond[1] && cond[0] > cond[2]) { const base = [(i + 1) % 3, (i + 2) % 3];
                ...
                drawRectangle(((centerX / 2) + 0)   - (mouseDist / 2), centerY - (cond[i != 0 ? 2 : 1] / 2), 50, cond[i != 0 ? 2 : 1], colours[base[0]]);
                drawRectangle(((centerX / 2) + 100) - (mouseDist / 2), centerY - (cond[i != 0 ? 1 : 2] / 2), 50, cond[i != 0 ? 1 : 2], colours[base[1]]);
            }
        });
