    ...
    function render() {
        ...
        [[pDist, qDist, rDist], [qDist, pDist, rDist], [rDist, pDist, qDist]].forEach((cond, i) => {
            if (cond[0] > cond[1] && cond[0] > cond[2]) { const bases = [(i + 1) % cond.length, (i + 2) % cond.length];
                ...
                drawRectangle(((centerX / 2) + 0)   - (mouseDist / 2), centerY - (pDist / 2), 50, cond[bases[1]], colours[bases[1]]);
                drawRectangle(((centerX / 2) + 100) - (mouseDist / 2), centerY - (pDist / 2), 50, cond[bases[2]], colours[bases[2]]);
            }
        });
