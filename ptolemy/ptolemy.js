window.onload = () => {
    ...
    function render() {
        ...
        [[pDist, qDist, rDist], [qDist, pDist, rDist], [rDist, qDist, pDist]].forEach((cond, i) => {
            if (cond[0] > cond[1] && cond[0] > cond[2]) { const base = [(i + 0) % cond.length, (i + 1) % cond.length, (i + 2) % cond.length];
                drawText('+', ((centerX / 2) + 75)  - (mouseDist / 2), centerY);
                drawText('=', ((centerX / 2) + 175) - (mouseDist / 2), centerY);
                drawRectangle(((centerX / 2) + 200) - (mouseDist / 2), centerY - (cond[base[0]] / 2), 50, cond[base[0]], colours[i]);
                drawRectangle(((centerX / 2) + 0)   - (mouseDist / 2), centerY - (cond[base[1]] / 2), 50, cond[base[1]], colours[base[1]]);
                drawRectangle(((centerX / 2) + 100) - (mouseDist / 2), centerY - (cond[base[2]] / 2), 50, cond[base[2]], colours[base[2]]);
            }
        });
